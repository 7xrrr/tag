...
export default {
	...
	function: async function (message: Message) {		
		if (message.author.bot || !message.content || !message.guild || message.channel.type !== ChannelType.GuildText || config.debugMode && !config.developers.includes(message.author.id) || (config.whitelistedGuildsOnly && !config.whiteListedGuilds.includes(message.guildId))) return 
